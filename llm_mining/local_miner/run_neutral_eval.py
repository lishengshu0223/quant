...
对指定因子(默认 F003 AmountStability_20D)做行业市值中性化后重新评价:
...
用法:
  python -m llm_mining.local_miner.run_neutral_eval --series F003
"""
...
def main():
    ...
    p.add_argument("--series", type=str, default="F003", help="因子系列ID")
    ...
    exp, ind_cols = factor_eval.load_barra_exposure(start_date=cfg.eval_start_date)
    console.log(f"    Barra 暴露加载完成: {len(exp)} 条记录, {len(ind_cols)} 个行业哑变量, "
                f"耗时 {time.time()-t0:.1f} 秒")
    t0 = time.time()
    neutral_wide = factor_eval.neutralize_factor(factor_wide, exp, ind_cols)
    del exp
