...
同时运行两个挖掘程序(optimize D001 + new), 任一进程异常退出时自动重启续传:
  - optimize 模式跑满 max_rounds 正常结束(输出最终报告图片)后不再重启;
  - new 模式挖到入库因子或跑满轮数正常结束时不再重启;
  - 仅当进程以非零退出码(LLM故障/网络错误)结束时自动重启续传。

用法:
  python -m llm_mining.tools.run_both_watchdog
  或直接 python llm_mining/tools/run_both_watchdog.py
"""
...
def main():
    print("=" * 60, flush=True)
    print("双程序守护启动: optimize D001 + new 挖掘", flush=True)
